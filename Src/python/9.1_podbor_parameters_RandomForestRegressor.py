...
for j in range(len(task)):
 ...
 # минимальное число объектов, необходимое для того, чтобы узел дерева мог бы расщепится
 min_samples_split = [int(x) for x in np.linspace(start = 2, stop = 50, num = 10)]
 # минимальное чмсло объектов в листьях
 ...
 for score in scores:
    ...
    clf = GridSearchCV(
        RandomForestRegressor(max_features=None), tuned_parameters, scoring='%s' % score, cv = 10)
